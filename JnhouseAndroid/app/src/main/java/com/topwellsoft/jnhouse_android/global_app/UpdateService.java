...
public class UpdateService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        //设置任务栏中下载进程显示的views
        views = new RemoteViews(getPackageName(), R.layout.checkupdate_update);
        notification.contentView = views;

        PendingIntent contentIntent = PendingIntent.getActivity(this, 0, new Intent(this, MainTabsActivity.class), 0);
        ...
        //启动线程开始执行下载任务
       downFile(GlobalObjects.urlPrefix+"/appPackage/CustomerApp.apk");
        ...
        new Thread() {
            public void run() {
                try {
                    ...
                    if (is != null) {
                        File rootFile = new File(Environment.getExternalStorageDirectory(), "/update");
                        if (!rootFile.exists() && !rootFile.isDirectory())
                            rootFile.mkdir();

                        tempFile = new File(Environment.getExternalStorageDirectory(),

                                "/update/" + url.substring(url.lastIndexOf("/") + 1));
                        if (tempFile.exists())
                            tempFile.delete();
            ...
            if (msg != null) {
                switch (msg.what) {
                    ...
                    case 3:
                        ...
                        //更新状态栏上的下载进度信息
                        views.setTextViewText(R.id.checkupdate_progress_text, "已下载" + download_precent + "%");
                        views.setProgressBar(R.id.checkupdate_progress, 100, download_precent, false);
                        notification.contentView = views;
                        nm.notify(notificationId, notification);
                        break;
                    case 4:
