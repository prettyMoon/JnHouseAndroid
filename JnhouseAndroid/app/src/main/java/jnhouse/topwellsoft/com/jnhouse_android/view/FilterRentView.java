...
/**
 * Created by chenchen on 2016/8/23.
 */
public class FilterRentView extends LinearLayout implements View.OnClickListener {
    ...
    private FilterEntity selectedRentTypeEntity; // 被选择的筛选项
    ...
    private List<FilterEntity> filterEntityList = ModelUtil.getRentType();
    ...
    private List<String>  mList7= new ArrayList<String>();//用来存放选中的数据
    ...
    private List<Integer> list7 = new ArrayList<Integer>();
    ...
    private HouseTowardAdapter houseTowardAdapter7;
    public FilterRentView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public FilterRentView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        View view = LayoutInflater.from(context).inflate(R.layout.view_filter_rent_layout, this);
    ...
    private void initListener() {
        ...
        llContentListView.setOnTouchListener(new View.OnTouchListener() {
            @Override
    ...
    private void setSortAdapter() {
        ...
        second_search_price_sure.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void setFilterOneAdapter() {
        ...
        lvLeft.setVisibility(GONE);
        lvRight.setVisibility(GONE);
        filterAdapter = new FilterOneAdapter(mContext, filterEntityList);
        second_search_type.setAdapter(filterAdapter);
        second_search_type.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                selectedRentTypeEntity = filterData.getRentType().get(position);
                filterAdapter.setSelectedEntity(selectedRentTypeEntity);
                tvFilter.setText(selectedRentTypeEntity.getKey());
                hide();
                if (onItemFilterClickListener != null) {
                    onItemFilterClickListener.onItemFilterClick(selectedRentTypeEntity );
                }
...
//        });
    ...
    private void setMoreAdapter() {
        ...
        gridView5.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                selectedlistSortEntity = filterData.getHouseType().get(position);
                int id = position;
                if (list5.contains(position)) {
                    for (int i = 0; i < list5.size(); i++) {
                        if (position == list5.get(i)) {
                            list5.remove(i);
                            mList5.remove(selectedlistSortEntity.getValue());
                        }
                    }
                } else {
                    list5.clear();
                    ...
                    list5.add(position);
                    //保存选中的数据
                    mList5.add(selectedlistSortEntity.getValue());
                }
        ...
        houseTowardAdapter7 = new HouseTowardAdapter(filterData.getFitment(), list7, mContext);
        gridView7.setAdapter(houseTowardAdapter7);
        gridView7.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                selectedlistSortEntity = filterData.getFitment().get(position);
                int id = position;
                if (list7.contains(position)) {
                    for (int i = 0; i < list7.size(); i++) {
                        if (position == list7.get(i)) {
                            list7.remove(i);
                            mList7.remove(selectedlistSortEntity.getValue());
                        }
                    }
                } else {
                    list7.clear();
                    ...
                    list7.add(position);
                    //保存选中的数据
                    mList7.add(selectedlistSortEntity.getValue());
                }
                houseTowardAdapter7.notifyDataSetChanged();
            }
        });
        ...
        gridView_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (onMoreFilterClickListener != null) {
                    onMoreFilterClickListener.onMoreFilterClick(mList1, mList2, mList3, mList4, mList5, mList6,mList7);
                }
            }
        });
        ...
        gridView_empty.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                list1.clear();
                list2.clear();
...
//    }
    ...
    public interface OnMoreFilterClickListener {
        void onMoreFilterClick(List<String> mList1, List<String> mList2, List<String> mList3, List<String> mList4, List<String> mList5, List<String> mList6,List<String>mList7);
    }
}
