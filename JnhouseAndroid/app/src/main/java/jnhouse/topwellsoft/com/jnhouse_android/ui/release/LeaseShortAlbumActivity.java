 ...
 */
public class LeaseShortAlbumActivity extends Activity {
    //显示手机里的所有图片的列表控件
    ...
    private class PreviewListener implements View.OnClickListener {
        public void onClick(View v) {
            if (LeaseShortBimp.tempSelectBitmap_leaseshort.size() > 0) {
                intent.putExtra("position", "1");
                intent.setClass(LeaseShortAlbumActivity.this, LeaseShortGalleryActivity.class);
    ...
    private class BackListener implements View.OnClickListener {
        public void onClick(View v) {
            intent.setClass(LeaseShortAlbumActivity.this, LeaseShortImageFile.class);
            startActivity(intent);
        }
    }
    ...
    private void init() {
        ...
        gridImageAdapter = new AlbumGridViewAdapter(this,dataList,
                LeaseShortBimp.tempSelectBitmap_leaseshort);
        gridView.setAdapter(gridImageAdapter);
        tv = (TextView) findViewById(Res.getWidgetID("myText"));
        gridView.setEmptyView(tv);
        okButton = (Button) findViewById(Res.getWidgetID("ok_button"));
        okButton.setText(Res.getString("finish")+"(" + LeaseShortBimp.tempSelectBitmap_leaseshort.size()
                + "/"+PublicWay.num+")");
    }
        ...
        gridImageAdapter
                .setOnItemClickListener(new AlbumGridViewAdapter.OnItemClickListener() {
                    ...
                    public void onItemClick(final ToggleButton toggleButton,
                        ...
                        if (LeaseShortBimp.tempSelectBitmap_leaseshort.size() >= PublicWay.num) {
                            toggleButton.setChecked(false);
                            chooseBt.setVisibility(View.GONE);
                            if (!removeOneData(dataList.get(position))) {
                                Toast.makeText(LeaseShortAlbumActivity.this, Res.getString("only_choose_num"),
                        ...
                        if (isChecked) {
                            ...
                            LeaseShortBimp.tempSelectBitmap_leaseshort.add(dataList.get(position));
                            okButton.setText(Res.getString("finish")+"(" + LeaseShortBimp.tempSelectBitmap_leaseshort.size()
                                    + "/"+PublicWay.num+")");

                        } else {
                            LeaseShortBimp.tempSelectBitmap_leaseshort.remove(dataList.get(position));
                            chooseBt.setVisibility(View.GONE);
                            okButton.setText(Res.getString("finish")+"(" + LeaseShortBimp.tempSelectBitmap_leaseshort.size() + "/"+PublicWay.num+")");
                        }
                        isShowOkBt();
                    }
                });
    ...
    private boolean removeOneData(ImageItem imageItem) {
        if (LeaseShortBimp.tempSelectBitmap_leaseshort.contains(imageItem)) {
            LeaseShortBimp.tempSelectBitmap_leaseshort.remove(imageItem);
            okButton.setText(Res.getString("finish")+"(" +LeaseShortBimp.tempSelectBitmap_leaseshort.size() + "/"+PublicWay.num+")");
            return true;
        }
        return false;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            intent.setClass(LeaseShortAlbumActivity.this, LeaseShareImageFile.class);
            startActivity(intent);
        }
        return false;
