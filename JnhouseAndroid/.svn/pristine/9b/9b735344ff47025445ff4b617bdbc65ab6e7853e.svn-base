 ...
 */
public class FilterNewHouseView extends LinearLayout implements View.OnClickListener {
    @Bind(R.id.tv_area)
    TextView tvCategory;
    ...
    private List<String> mList7 = new ArrayList<String>();
    ...
    public FilterNewHouseView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public FilterNewHouseView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        View view = LayoutInflater.from(context).inflate(R.layout.view_filter_new_house_layout, this);
...
//        });
    ...
    private void setMoreAdapter() {
        ...
        houseTowardAdapter3 = new HouseTowardAdapter(filterData.getBorough_properties(), list3, mContext);
        gridView3.setAdapter(houseTowardAdapter3);
        gridView3.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                selectedareaTypEntity = filterData.getBorough_properties().get(position);
                int id = position;
        ...
        houseTowardAdapter4 = new HouseTowardAdapter(filterData.getLine(), list4, mContext);
        gridView4.setAdapter(houseTowardAdapter4);
        gridView4.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                selectedageTypEntity = filterData.getLine().get(position);
                int id = position;
        ...
        gridView5.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                selectedhouseTypEntity = filterData.getHouseType().get(position);
                int id = position;
                if (list5.contains(position)) {
                    for (int i = 0; i < list5.size(); i++) {
                        if (position == list5.get(i)) {
                            list5.remove(i);
                            mList5.remove(selectedhouseTypEntity.getValue());
                        }
                    }
                } else {
                    list5.clear();
                    ...
                    list5.add(position);
                    //保存选中的数据
                    mList5.add(selectedhouseTypEntity.getValue());
                }
    ...
    // 设置筛选数据
    public void setFilterData(Activity activity, FilterNewHouseData filterData) {
