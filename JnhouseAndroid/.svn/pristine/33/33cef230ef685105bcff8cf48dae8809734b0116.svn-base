...
import jnhouse.topwellsoft.com.jnhouse_android.ui.chat.activity.ease_activity.EaseShowVideoActivity;
    ...
    protected void onSetUpView() {
        ...
        if (message.direct() == EMMessage.Direct.RECEIVE) {
            if (videoBody.thumbnailDownloadStatus() == EMFileMessageBody.EMDownloadStatus.DOWNLOADING ||
                ...
                // System.err.println("!!!! not icon_titlebar_back receive, show image directly");
                imageView.setImageResource(R.drawable.ease_default_image);
                if (localThumb != null) {
                    showVideoThumbView(localThumb, imageView, videoBody.getThumbnailUrl(), message);
                }
