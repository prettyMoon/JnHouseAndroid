...
public class SecondHouseAdapter extends BaseListAdapter<SecondHouseEntity> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String roomType = entity.getHouse_room() + " " + entity.getHouse_totalarea() + "m²";
        if (entity.getHouse_toward() != null)
            roomType = roomType + " " + entity.getHouse_toward();
        holder.tvRoom.setText(roomType);
        holder.tvPrice.setText(entity.getHouse_price().replace(",","") + "万");
        holder.tvName.setText(entity.getBorough_name());
        if (entity.getHouse_jj().length() > 0 && !entity.getHouse_jj().equals("0"))
            holder.tvHousejj.setText(entity.getHouse_jj().replace(",","") + "元/m²");
        else holder.tvHousejj.setText("价格未知");
